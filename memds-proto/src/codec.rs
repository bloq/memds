...
impl MemdsCodec {
    pub fn new() -> MemdsCodec {
        MemdsCodec { state: DecodeState::Head }
    }
    ...
    fn decode_data(&mut self, msg_size: usize, src: &mut BytesMut) -> Result<Option<MemdsMessage>, MemdsError> {
        if src.len() < msg_size {
	    return Ok(None)
	}
