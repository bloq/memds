...
#[cfg(test)]
mod tests {
    use crate::memds_api::{AtomType, DbValue, MemdsMessage, MemdsMessage_MsgType};
    use crate::{MemdsCodec, MemdsError};
    use bytes::BytesMut;
    use tokio_util::codec::{Decoder, Encoder};
    ...
    fn basic_codec() {
        ...
        dbv.set_key(b"foo".to_vec());
        dbv.set_str(b"bar".to_vec());
        let mut enc_msg = MemdsMessage::new();
        enc_msg.mtype = MemdsMessage_MsgType::DBVAL;
        enc_msg.set_dbv(dbv);
